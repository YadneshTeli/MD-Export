    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.startsWith('```')) {
            ...
            if (codeBlock === null) {
                ...
                // Fix duplicate language labels generated by Claude/Gemini scrapers
                // Account for an optional 'break' segment between the label paragraph and the code block
                let popCount = 0;
                let lastSeg = segs[segs.length - 1];
                if (lastSeg && lastSeg.type === 'break') {
                    popCount = 1;
