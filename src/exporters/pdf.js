...
export async function toPdf(processed) {
    const { title, site, stats, messages } = processed;
    ...
    setColor(colors.userBar, 'fill');
    doc.rect(0, 54, PAGE_W / 2, 2, 'F');
    setColor(colors.botBar, 'fill');
    doc.rect(PAGE_W / 2, 54, PAGE_W / 2, 2, 'F');
    ...
    for (let i = 0; i < messages.length; i++) {
        ...
        setColor(colors.textMuted);
        doc.text(
            `${wordCount} words${hasCode ? '  ·  ' + codeLanguages.join(', ') : ''}`,
            PAGE_W - MARGIN - 2,
            headerY + 7,
            { align: 'right' },
        );
        ...
        renderSegments(doc, Y, segments, MARGIN + 5, CONTENT_W - 10, setColor, wrapText, needsPage);
    ...
    for (const seg of segments) {
        switch (seg.type) {
            ...
            case 'blockquote': {
                ...
                drawInlineRuns(doc, Y, seg.text, x + 5, maxW - 5, 8.5, setColor, { r: 80, g: 80, b: 90 }, needsPage);
                break;
            }
            ...
            case 'bullet': {
                ...
                doc.text('•', x, Y.value);
                drawInlineRuns(doc, Y, seg.text, x + 4, maxW - 4, 8.5, setColor, colors.textPrimary, needsPage);
                break;
            }
            ...
            case 'numbered': {
                ...
                drawInlineRuns(doc, Y, seg.text, x + 6, maxW - 6, 8.5, setColor, colors.textPrimary, needsPage);
                break;
            }

            case 'paragraph': {
                needsPage(6);
                drawInlineRuns(doc, Y, seg.text, x, maxW, 8.5, setColor, colors.textPrimary, needsPage);
            ...
            case 'code': {
                ...
                const allLines = rawLines.flatMap(l => (l ? doc.splitTextToSize(sanitizePdfText(l), maxW - 8) : ['']));
            ...
            case 'table': {
                ...
                const fitCell = text => {
                    ...
                    while (t.length > 0 && doc.getTextWidth(t + '…') > colW - 4) t = t.slice(0, -1);
                    return t + '…';
                };
